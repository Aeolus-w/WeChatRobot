...
def search_knowledge_db(session_id, query, top_k=2):
    vector_kb_folder = f"/root/autodl-tmp/base_knowledge/{session_id}/vector_kb"
...
async def chat_completions(body: ChatCompletionBody, request: Request):
    ...
    if (
        ...
        return await chat(model, body, request, completion_text, session_id)
...
async def chat(
    model: TextRWKV, body: ChatCompletionBody, request: Request, completion_text: str, session_id: str
):
    user_input = body.messages[-1].content  # 直接访问 content
    print(user_input)

    # 检查是否以“查询：”开头
    if user_input.startswith("查询："):
        query = user_input[len("查询："):].strip()  # 去掉“查询：”获取实际查询内容
        search_result = search_knowledge_db(session_id, query)
        
        if search_result.strip():
            # 如果找到结果，直接返回检索内容
            response = f"检索到以下相关信息：\n\n{search_result}"
        else:
            # 如果没有找到结果，明确提示
            response = "没有找到相关信息，请检查查询内容或上传相关文档。"
    ...
    # 检查是否以“询问：”开头
    if user_input.startswith("询问："):
        query = user_input[len("询问："):].strip()  # 去掉“询问：”获取实际查询内容
        search_result = search_knowledge_db(session_id, query)
        print(search_result)
        if search_result:
            # 如果找到相关附属知识，将附属知识和用户的查询内容一起发送给模型
            response_content = f"用户问题是：{query}\n\n在用户知识库中可以找到的知识是：\n\n{search_result}\n\n请你根据这些知识回答用户的问题，尽量完整通顺。"
        else:
            # 如果没有找到相关附属知识，返回提示信息
            response_content = "没有找到相关附属知识，请检查查询内容或上传相关文档。"

        # 使用原来的模型生成回复逻辑
        if body.stream:
            return EventSourceResponse(
                eval_rwkv(
                    model, request, body, response_content, body.stream, body.stop, True
                )
            )
        else:
            try:
